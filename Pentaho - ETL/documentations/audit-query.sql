       ...
       (
        ...
        SELECT 2 AS group_seq, 'target' AS table_type, 'fact_orders date_diff' AS table_name, SUM(ship_date_skey - order_date_skey) AS audit_value
         ...
         UNION ALL
          ...
          /* State Information */
        SELECT 3 AS group_seq, 'source' AS table_type, 'stg_state count' AS table_name, COUNT(*) AS audit_value
        ...
        SELECT 4 AS group_seq, 'source' AS table_type, 'stg_customer count' AS table_name, COUNT(*) AS audit_value
          FROM stage_db.stg_customer
         UNION ALL
        SELECT 4 AS group_seq, 'target' AS table_type, 'dim_customer count' AS table_name, COUNT(*) AS audit_value
          FROM datamart_db.dim_customer

         UNION ALL

          /* Address Information */
        SELECT 5 AS group_seq, 'source' AS table_type, 'stg_address count' AS table_name, COUNT(*) AS audit_value
          FROM stage_db.stg_address
         UNION ALL
        SELECT 5 AS group_seq, 'target' AS table_type, 'dim_address count' AS table_name, COUNT(*) AS audit_value
          FROM datamart_db.dim_address
       ) a
 GROUP BY group_seq
